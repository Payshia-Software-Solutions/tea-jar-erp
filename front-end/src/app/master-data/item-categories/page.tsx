...
import { createItemCategory, deleteItemCategory, fetchItemCategories, updateItemCategory, type ItemCategory } from "@/lib/api";
import { Plus, Search, Trash2, Pencil, Loader2, AlertCircle, Grid } from "lucide-react";
...
export default function ItemCategoriesPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        await updateItemCategory(String(editId), { name: trimmed });
        toast({ title: "Updated", description: "Category updated" });
      } else {
        await createItemCategory({ name: trimmed });
  ...
  const remove = async (id: number) => {
    ...
    try {
      await deleteItemCategory(String(id));
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Item Categories</h1>
          <p className="text-muted-foreground mt-1">Common item classification</p>
        </div>
            ...
            <DialogContent className="sm:max-w-[425px]">
              <form onSubmit={submit}>
                <DialogHeader>
                  ...
                  <DialogDescription>e.g., Spare Parts, Accessories, Consumables.</DialogDescription>
                </DialogHeader>
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-muted-foreground max-w-xs">{query ? "No results match your search." : "Add your first category."}</p>
              </div>
            ) : (
              <Table>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">CATEGORY ID: #{b.id}</p>
                          </div>
                        </div>
                      </TableCell>
